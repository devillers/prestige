...
const schema = yup.object().shape({
  ...
  localisation: yup.string().when("type", {
    is: "demande",
    then: (s) => s.required("Localisation requise"),
  }),
  surface: yup
    .number()
    ...
    .when("type", {
      is: "demande",
      then: (s) => s.required("Surface requise").positive("Doit être > 0"),
    }),
  chambres: yup
    .number()
    .typeError("Chambres doit être un nombre entier")
    .when("type", {
      is: "demande",
      then: (s) => s.required("Chambres requises").integer("Entier requis").min(1),
    }),
  sallesDeBain: yup
    .number()
    .typeError("Salles de bain doit être un nombre entier")
    .when("type", {
      is: "demande",
      then: (s) => s.required("Salles de bain requises").integer("Entier requis").min(1),
    }),
});
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-7xl mx-auto bg-white mt-12">
        <div className="flex flex-col md:flex-row gap-8 text-sm">
          ...
          <div className="md:w-1/2 space-y-4">
            ...
            {/* ... reste inchangé (upload photos et bouton) ... */}
          </div>
        </div>
      </form>
      ...
      <style jsx>{`
        ...
        .error { color: #e53e3e; font-size: 0.75rem; }
        .icon-link { /* inchangé */ }
        .btn-submit { /* inchangé */ }
      `}</style>
    </>
